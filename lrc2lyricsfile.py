...
(.lyricsfile.yaml) document, compatible with the spec in lyricsfile/SPECIFICATION.md.
...
def parse_lrc(content):
    ...
    for raw_line in content.splitlines():
        ...
        # parse line-level timestamp: [mm:ss.mmm]
        line_ts_match = re.match(r'^\[(\d{1,2}):(\d{1,2})\.(\d{1,3})\](.*)', line)
        ...
        # parse word-level timestamps: <mm:ss.mmm>word<mm:ss.mmm>word...
        words = []
        word_pattern = re.compile(r'<(\d{1,2}):(\d{1,2})\.(\d{1,3})>([^<]*)')
    ...
    # set each line's end_ms = next line's start_ms
...
def main():
    ...
    if not os.path.isfile(args.lrc_path):
        print(f"[ERROR] File not found: {args.lrc_path}", file=sys.stderr)
        sys.exit(1)

    out_dir = os.path.dirname(os.path.abspath(args.lrc_path))
    base = os.path.splitext(os.path.basename(args.lrc_path))[0]
    out_path = os.path.join(out_dir, base + ".lyricsfile.yaml")

    # Resolve metadata (audio priority, manual fallback)
    audio = find_audio(args.lrc_path)
    ...
    print(f"  lines: {len(parsed['lines'])}, "
          f"words: {sum(len(l['words']) for l in parsed['lines'])}, "
